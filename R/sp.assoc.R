sp.assoc <-
   function(matr)
   {
      ...
      n <-
         apply(matr, 2, sum) ## Number of plots occupied by certain species
      Tj <-
         apply(matr, 1, sum) ## Total number of species for each plot
      t <- mean(Tj) ## Mean species number for all the plots
      STsq <- (1 / N) * sum((Tj - t) ^ 2) ## Variance of species number
      sigmaTsq <-
         sum((1 - frq) * frq) ## Variance of species relative frequency
      VR <-
         STsq / sigmaTsq ##Variance ratio: VR>1 Positively associated, VR<1 Negative associated
